...
import com.light.upgrade.network.NetworkUtils;
import com.light.upgrade.utils.SharedPreferencesUtils;
...
public class UpgradeHelper {
    ...
    public void run(final UpgradeCheckCallback back) {
        if (NetworkUtils.isNetworkAvailable(mContext)) {
            if (null != broadcastReceiver && isRegisterBR) {
                mContext.unregisterReceiver(broadcastReceiver);
                isRegisterBR = false;
            }
            new CheckAsyncTask(mContext,back).setConfig(config).execute();
        } else {
            if (!isRegisterBR) {
